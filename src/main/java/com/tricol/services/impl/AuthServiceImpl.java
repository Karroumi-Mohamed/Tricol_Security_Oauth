...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        UserApp user = UserApp.builder()
                ...
                .password(passwordEncoder.encode(request.getPassword())) // Added encode call logic which was in original but simpler
                // Wait, original read_file said: password(request.getPassword()) then in DataSeeder it used encode.
                // Ah, check previous read of AuthService:
                // .password(request.getPassword()) -> Checks source:
                // `AuthenticationManager` might need encoded, but `UserApp` stores it. The `loadUserByUsername` checks against it.
                // Wait, typically we encode before saving. The previous code MIGHT have missed encode call?
                ...
                // Wait, if I change logic now i might break something, but storing plain text passwords is BAD.
